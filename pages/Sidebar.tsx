...
const Sidebar: React.FC = () => {
  ...
  const mainLinks = [
    ...
    { to: '/chat', text: 'Global Chat', icon: 'M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a2 2 0 01-2-2V10a2 2 0 012-2h8z' },
  ...
  const renderLink = (link: { to: string; text: string; icon: string; }) => (
    <div
      key={link.to}
      className="relative"
    >
  ...
  return (
    <div className={`bg-blue-900 text-white flex flex-col transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'}`}>
      ...
      <nav className="flex-1 px-2 py-4 flex flex-col justify-between">
        ...
        <div>
          {userPermissions.canAccessSettings && renderLink(settingsLink)}
        </div>
      </nav>
    </div>
  );
