  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex justify-end gap-2 mt-6">
          <button onClick={() => setIsDeleteModalOpen(false)} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 font-medium" disabled={isDeleting}>
            {canDeleteTeamCheck().canDelete ? 'Cancel' : 'Close'}
          </button>
          {canDeleteTeamCheck().canDelete && (
            <button onClick={handleDeleteTeam} className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-medium disabled:opacity-50 flex items-center" disabled={isDeleting}>
              {isDeleting && (
